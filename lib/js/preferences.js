...
$(document).ready(function(){
	...
	$('.sectionLink').click(function(e){
		...
		$('#prefsMainPane').load('lib/html/preferences/'+pref_section+'.html', function(){
			// Load the JS for the page, if available
			global.Loader.loadJS('lib/js/preferences/'+pref_section+'.js')
			...
			// Add a class to the prefsMainPane to indicate which page we're on
			$('#prefsMainPane').removeClassRegex(/^section_/).addClass('section_'+pref_section)


			// Fill in the input fields with existing values
			$("#prefsMainPane :input[type!='file']").each(function(){
				if ($(this).hasClass('allValues')) {
